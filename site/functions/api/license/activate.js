...
//
// Canonical path: /api/license/activate (renamed 2026-08-20 — the paid tier was
// once branded "STREETS" and this lived at /api/streets/activate, which kept
// confusing readers into treating it as retired-curriculum leftovers).
// The legacy path is a re-export shim (functions/api/streets/activate.js) kept
// for pre-rename desktop builds whose recovery URL is baked into the binary;
// see that file for when it may be deleted.
//
...
// streets.4da.ai was removed 2026-08-20: the subdomain is NXDOMAIN (verified
// against 1.1.1.1) and nothing has served from it since STREETS retired.
const BASE_ORIGINS = ['https://4da.ai', 'https://www.4da.ai', 'tauri://localhost'];
...
// by a broken counter store. A WAF rate-limiting rule on /api/license/activate
// (and the legacy /api/streets/activate shim, while it exists) remains
// worthwhile belt-and-braces (dashboard-only, zone-level).
//
