// GAME Component: source-vitals — auto-generated, do not edit.
(function(){
...
fn sdf_hex(p: vec2<f32>, r: f32) -> f32 {
    let k = vec3<f32>(-0.866025, 0.5, 0.577350);
    var q = abs(p);
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let health_avg = u.p_health_avg;
    let active_ratio = u.p_active_ratio;
    let error_rate = u.p_error_rate;
    ...
    // ── Layer 0: cells ──
    {
        ...
        var sdf_result = sdf_hex(p, 0.350000);
        let glow_pulse = 0.600000 * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(health_avg, health_avg, health_avg), color_result.a);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * 3.000000 + vec2<f32>(0.0, 1.3), i32(3.000000), 0.200000, 2.000000);
        let warp_y = fbm2(p * 3.000000 + vec2<f32>(1.7, 0.0), i32(3.000000), 0.200000, 2.000000);
        ...
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.080000, 0.100000, 0.080000), vec3<f32>(active_ratio, 0.300000, 0.150000), vec3<f32>(0.600000, 0.900000, 0.500000), vec3<f32>(0.000000, 0.200000, 0.100000));
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * 2.000000 + vec2<f32>(0.0, 1.3), i32(2.000000), 0.300000, 2.000000);
        let warp_y = fbm2(p * 2.000000 + vec2<f32>(1.7, 0.0), i32(2.000000), 0.300000, 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.300000; }
        var sdf_result = noise2(p * 1.500000 + vec2<f32>(time * 0.1, time * 0.07));
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(error_rate, 0.020000, 0.020000), vec3<f32>(0.500000, 0.100000, 0.050000), vec3<f32>(1.000000, 0.200000, 0.100000), vec3<f32>(0.000000, 0.000000, 0.000000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let la = color_result.a;
        let lc = color_result.rgb;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    ...
    // ── Layer 3: core ──
    {
        ...
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.600000, 0.900000, 0.500000), color_result.a);
        let la = color_result.a;
        let lc = color_result.rgb;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
...
`;
const UNIFORMS = [{name:'health_avg',default:0},{name:'active_ratio',default:0},{name:'error_rate',default:0}];
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let vign = 1.0 - 0.500000 * length(uv - 0.5);
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'health_avg', target: 'cells', field: 'brightness', weight: 0.5 },
      { source: 'error_rate', target: 'error_signal', field: 'brightness', weight: 0.8 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class SourceVitals extends HTMLElement {
  constructor() {
    super();
  ...
  get health_avg() { return this._renderer?.userParams['health_avg'] ?? 0; }
  set health_avg(v) { this.setParam('health_avg', v); }
  get active_ratio() { return this._renderer?.userParams['active_ratio'] ?? 0; }
  set active_ratio(v) { this.setParam('active_ratio', v); }
  get error_rate() { return this._renderer?.userParams['error_rate'] ?? 0; }
  set error_rate(v) { this.setParam('error_rate', v); }
