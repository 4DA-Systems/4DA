// GAME Component: briefing-atmosphere — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let signal_heat = u.p_signal_heat;
    let decision_pressure = u.p_decision_pressure;
    let quality = u.p_quality;
    ...
    // ── Layer 0: fog ──
    {
        ...
        { let warp_x = fbm2(p * 2.000000 + vec2<f32>(0.0, 1.3), i32(3.000000), 0.100000, 2.000000);
        let warp_y = fbm2(p * 2.000000 + vec2<f32>(1.7, 0.0), i32(3.000000), 0.100000, 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.100000; }
        var sdf_result = fbm2((p * 3.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(4.000000), 0.600000, 2.000000);
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.040000, 0.040000, 0.060000), vec3<f32>(0.120000, 0.100000, 0.150000), vec3<f32>(0.600000, 0.500000, 0.700000), vec3<f32>(0.000000, 0.050000, 0.150000));
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * 3.000000 + vec2<f32>(0.0, 1.3), i32(2.000000), 0.150000, 2.000000);
        let warp_y = fbm2(p * 3.000000 + vec2<f32>(1.7, 0.0), i32(2.000000), 0.150000, 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.150000; }
        var sdf_result = noise2(p * 2.000000 + vec2<f32>(time * 0.1, time * 0.07));
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.000000, 0.000000, 0.000000), vec3<f32>(signal_heat, 0.200000, 0.050000), vec3<f32>(1.000000, 0.700000, 0.300000), vec3<f32>(0.000000, 0.100000, 0.200000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let la = color_result.a;
        let lc = color_result.rgb;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * 1.500000 + vec2<f32>(0.0, 1.3), i32(2.000000), 0.200000, 2.000000);
        let warp_y = fbm2(p * 1.500000 + vec2<f32>(1.7, 0.0), i32(2.000000), 0.200000, 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.200000; }
        var sdf_result = fbm2((p * 2.000000 + vec2<f32>(time * 0.1, time * 0.07)), i32(2.000000), 0.300000, 2.000000);
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(decision_pressure, 0.020000, 0.020000), vec3<f32>(0.300000, 0.050000, 0.050000), vec3<f32>(0.800000, 0.150000, 0.100000), vec3<f32>(0.000000, 0.000000, 0.000000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let la = color_result.a;
        let lc = color_result.rgb;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    ...
    // ── Layer 3: quality_glow ──
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        var sdf_result = sdf_circle(p, 0.500000);
        let glow_pulse = quality * (0.9 + 0.1 * sin(time * 2.0));
        let glow_result = apply_glow(sdf_result, glow_pulse);
        var color_result = vec4<f32>(vec3<f32>(glow_result), glow_result);
        color_result = vec4<f32>(color_result.rgb * vec3<f32>(0.900000, 0.850000, 0.750000), color_result.a);
        let la = color_result.a;
        let lc = color_result.rgb;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
...
uniform float u_p_signal_heat;
uniform float u_p_decision_pressure;
uniform float u_p_quality;
...
void main(){
    ...
    float signal_heat = u_p_signal_heat;
    float decision_pressure = u_p_decision_pressure;
    float quality = u_p_quality;
    ...
    // ── Layer 0: fog ──
    {
        ...
        { float warp_x = fbm2(p * 2.000000 + vec2(0.0, 1.3), int(3.000000), 0.100000, 2.000000);
        float warp_y = fbm2(p * 2.000000 + vec2(1.7, 0.0), int(3.000000), 0.100000, 2.000000);
        p = p + vec2(warp_x, warp_y) * 0.100000; }
        float sdf_result = fbm2((p * 3.000000 + vec2(time * 0.1, time * 0.07)), int(4.000000), 0.600000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.040000, 0.040000, 0.060000), vec3(0.120000, 0.100000, 0.150000), vec3(0.600000, 0.500000, 0.700000), vec3(0.000000, 0.050000, 0.150000));
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float warp_x = fbm2(p * 3.000000 + vec2(0.0, 1.3), int(2.000000), 0.150000, 2.000000);
        float warp_y = fbm2(p * 3.000000 + vec2(1.7, 0.0), int(2.000000), 0.150000, 2.000000);
        p = p + vec2(warp_x, warp_y) * 0.150000; }
        float sdf_result = noise2(p * 2.000000 + vec2(time * 0.1, time * 0.07));
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.000000, 0.000000, 0.000000), vec3(signal_heat, 0.200000, 0.050000), vec3(1.000000, 0.700000, 0.300000), vec3(0.000000, 0.100000, 0.200000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        float la = color_result.a;
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float warp_x = fbm2(p * 1.500000 + vec2(0.0, 1.3), int(2.000000), 0.200000, 2.000000);
        float warp_y = fbm2(p * 1.500000 + vec2(1.7, 0.0), int(2.000000), 0.200000, 2.000000);
        p = p + vec2(warp_x, warp_y) * 0.200000; }
        float sdf_result = fbm2((p * 2.000000 + vec2(time * 0.1, time * 0.07)), int(2.000000), 0.300000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(decision_pressure, 0.020000, 0.020000), vec3(0.300000, 0.050000, 0.050000), vec3(0.800000, 0.150000, 0.100000), vec3(0.000000, 0.000000, 0.000000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        float la = color_result.a;
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    ...
    // ── Layer 3: quality_glow ──
    {
        ...
        float glow_pulse = quality * (0.9 + 0.1 * sin(time * 2.0));
        float glow_result = apply_glow(sdf_result, glow_pulse);

        vec4 color_result = vec4(vec3(glow_result), glow_result);
        color_result = vec4(color_result.rgb * vec3(0.900000, 0.850000, 0.750000), color_result.a);
        float la = color_result.a;
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    }
...
`;
const UNIFORMS = [{name:'signal_heat',default:0},{name:'decision_pressure',default:0},{name:'quality',default:0}];
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'quality', target: 'fog', field: 'brightness', weight: 0.4 },
      { source: 'signal_heat', target: 'heat', field: 'brightness', weight: 0.6 },
      { source: 'decision_pressure', target: 'pressure', field: 'brightness', weight: 0.7 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class BriefingAtmosphere extends HTMLElement {
  constructor() {
    super();
  ...
  get signal_heat() { return this._renderer?.userParams['signal_heat'] ?? 0; }
  set signal_heat(v) { this.setParam('signal_heat', v); }
  get decision_pressure() { return this._renderer?.userParams['decision_pressure'] ?? 0; }
  set decision_pressure(v) { this.setParam('decision_pressure', v); }
  get quality() { return this._renderer?.userParams['quality'] ?? 0; }
  set quality(v) { this.setParam('quality', v); }
