// GAME Component: playbook-pathway — auto-generated, do not edit.
(function(){
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let momentum = u.p_momentum;
    let streak = u.p_streak;
    ...
    // ── Layer 0: river ──
    {
        ...
        { let warp_x = fbm2(p * 5.000000 + vec2<f32>(0.0, 1.3), i32(3.000000), 0.120000, 2.000000);
        let warp_y = fbm2(p * 5.000000 + vec2<f32>(1.7, 0.0), i32(3.000000), 0.120000, 2.000000);
        ...
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.060000, 0.060000, 0.060000), vec3<f32>(progress, 0.200000, 0.150000), vec3<f32>(0.800000, 0.650000, 0.300000), vec3<f32>(0.000000, 0.100000, 0.200000));
    ...
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        { let warp_x = fbm2(p * 2.000000 + vec2<f32>(0.0, 1.3), i32(2.000000), 0.080000, 2.000000);
        let warp_y = fbm2(p * 2.000000 + vec2<f32>(1.7, 0.0), i32(2.000000), 0.080000, 2.000000);
        p = p + vec2<f32>(warp_x, warp_y) * 0.080000; }
        var sdf_result = noise2(p * 1.500000 + vec2<f32>(time * 0.1, time * 0.07));
        let pal_rgb = cosine_palette(sdf_result, vec3<f32>(0.000000, 0.000000, 0.000000), vec3<f32>(0.200000, 0.150000, 0.100000), vec3<f32>(1.000000, 0.850000, 0.550000), vec3<f32>(0.000000, 0.050000, 0.100000));
        var color_result = vec4<f32>(pal_rgb, clamp(dot(pal_rgb, vec3<f32>(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        let la = color_result.a;
        let lc = color_result.rgb;
        final_color = vec4<f32>(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    ...
    // ── Layer 2: active_pulse ──
    {
        var p = vec2<f32>(uv.x * aspect, uv.y);
        var sdf_result = sdf_circle(p, 0.150000);
        let glow_pulse = momentum * (0.9 + 0.1 * sin(time * 2.0));
...
uniform float u_p_momentum;
uniform float u_p_streak;
...
void main(){
    ...
    float momentum = u_p_momentum;
    float streak = u_p_streak;
    ...
    // ── Layer 0: river ──
    {
        ...
        { float warp_x = fbm2(p * 5.000000 + vec2(0.0, 1.3), int(3.000000), 0.120000, 2.000000);
        float warp_y = fbm2(p * 5.000000 + vec2(1.7, 0.0), int(3.000000), 0.120000, 2.000000);
        p = p + vec2(warp_x, warp_y) * 0.120000; }
        float sdf_result = fbm2((p * 4.000000 + vec2(time * 0.1, time * 0.07)), int(3.000000), 0.450000, 2.000000);
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.060000, 0.060000, 0.060000), vec3(progress, 0.200000, 0.150000), vec3(0.800000, 0.650000, 0.300000), vec3(0.000000, 0.100000, 0.200000));
    ...
    {
        vec2 p = vec2(uv.x * aspect, uv.y);
        { float warp_x = fbm2(p * 2.000000 + vec2(0.0, 1.3), int(2.000000), 0.080000, 2.000000);
        float warp_y = fbm2(p * 2.000000 + vec2(1.7, 0.0), int(2.000000), 0.080000, 2.000000);
        p = p + vec2(warp_x, warp_y) * 0.080000; }
        float sdf_result = noise2(p * 1.500000 + vec2(time * 0.1, time * 0.07));
        vec3 pal_rgb = cosine_palette(sdf_result, vec3(0.000000, 0.000000, 0.000000), vec3(0.200000, 0.150000, 0.100000), vec3(1.000000, 0.850000, 0.550000), vec3(0.000000, 0.050000, 0.100000));
        vec4 color_result = vec4(pal_rgb, clamp(dot(pal_rgb, vec3(0.299, 0.587, 0.114)) * 2.0, 0.0, 1.0));
        float la = color_result.a;
        vec3 lc = color_result.rgb;
        final_color = vec4(final_color.rgb * (1.0 - la) + lc, final_color.a * (1.0 - la) + la);
    ...
    // ── Layer 2: active_pulse ──
    {
        ...
        float glow_pulse = momentum * (0.9 + 0.1 * sin(time * 2.0));
...
`;
const UNIFORMS = [{name:'progress',default:0},{name:'momentum',default:0},{name:'streak',default:0}];
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let vign = 1.0 - 0.400000 * length(uv - 0.5);
...
class GameResonanceNetwork {
  constructor() {
    this._couplings = [
      { source: 'progress', target: 'river', field: 'brightness', weight: 0.5 },
      { source: 'streak', target: 'active_pulse', field: 'brightness', weight: 0.4 },
    ];
    this._damping = 0.95;
    this._maxDepth = 4;
    this._state = new Map();
...
class GameArcTimeline {
  constructor() {
    ...
    this._entries = [
      { target: 'momentum', from: 0.4, to: 0.8, duration: 3, easing: 'ease_in_out' },
    ];
  }
...
class PlaybookPathway extends HTMLElement {
  constructor() {
    super();
  ...
  get momentum() { return this._renderer?.userParams['momentum'] ?? 0; }
  set momentum(v) { this.setParam('momentum', v); }
  get streak() { return this._renderer?.userParams['streak'] ?? 0; }
  set streak(v) { this.setParam('streak', v); }
