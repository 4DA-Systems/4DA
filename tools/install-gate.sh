#!/bin/bash
# Install the the external verifier local pre-push gate — WITHOUT clobbering an existing hook system.
#
...
# "external-verifier local gate" block from the active pre-push (printed below).
set -e
ROOT="$(git rev-parse --show-toplevel)"
GATE_REL=".githooks/pre-push"
[ -f "$ROOT/$GATE_REL" ] || { echo "[external-verifier-gate] no $GATE_REL in this repo — nothing to install"; exit 1; }
...
MARK="# >>> external-verifier local gate >>>"
END="# <<< external-verifier local gate <<<"
DELEGATE="bash \"\$(git rev-parse --show-toplevel)/$GATE_REL\" \"\$@\" || exit 1"

if [ -f "$HOOK" ] && grep -qF "$MARK" "$HOOK"; then
  echo "[external-verifier-gate] already installed in $KIND hook ($HOOK)."
  exit 0
fi
...
if [ -f "$HOOK" ]; then
  ...
  echo "[external-verifier-gate] CHAINED the gate onto the existing $KIND pre-push ($HOOK)."
else
  ...
  echo "[external-verifier-gate] installed a new $KIND pre-push ($HOOK)."
fi
chmod +x "$HOOK" 2>/dev/null || true
echo "  gate spec: .external-verifier/gate.json   ·   bypass: git push --no-verify (or EXTVERIFIER_SKIP_GATE=1)"
