...
- **Decision:** PIPELINE_VERSION 29 (2026-09-04), the remediation of the scoring-pipeline adversarial investigation (`.claude/plans/scoring-pipeline-adversarial-investigation-2026-09-04.md`). Four rules:
  ...
  2. **Any cap that sits above the relevance line is also a verdict gate.** Zero-engagement UGC (capped 0.50) and OFF-STACK ungrounded security advisories (capped 0.35; an ungrounded advisory on the user's own stack keeps the 0.44 awareness cap and is decided by score) are categorically not feed-relevant, exactly the v18 registry-release shape. A cap alone bounds RANK; membership is decided by the verdict.
