...
export default defineConfig({
  ...
  test: {
    ...
    pool: 'forks',
    // vitest 4 REMOVED `test.poolOptions`; every former pool option is now
    // top-level, and `poolOptions.forks.maxForks` is spelled `maxWorkers`
    // (verified against the installed 4.1.11 type surface, not guessed). The
    // config below was accepted-and-ignored for one run of this upgrade
    // branch, which would have silently restored the UNBOUNDED default fork
    // count — the exact condition d271809f set an explicit bound to fix. The
    // measured rationale for the value is unchanged and kept verbatim below.
    ...
    // NO worker memoryLimit. It was set to '512MB' and read as a safety net
    // during the 2026-08-31 runner-wedge investigation; it never was one.
    // Verified against vitest 3.2.6: `getWorkerMemoryLimit` read
    // `poolOptions.vmForks` / `vmThreads` — never `forks` — so the key was
    // accepted and silently ignored. Re-adding it buys nothing; a real worker
    // memory ceiling needs the `vmForks` pool, and that is an ADR.
    coverage: {
      provider: 'v8',
