...
project_health.rs (opus-scoring-external-verifier-fix), MCP osv work, peer dotfiles/.bak/ResultItemExpanded.tsx.
...
NOTE: my local commit d7fba7f4 lingers on the LOCAL fix/scoring-correctness-ext branch (= 872c41cf +
...
the external verifier board was 401 (no signing key this session) — coordinated via TERMINALS.md; proof = git SHA 76ed0ae2.
Ships SILENT (no UI). Next: Phase 2c (first visible surface) — gated behind founder dogfood.
**Commit Lock**: RELEASED.

### Terminal: opus-scoring-external-verifier-fix (started 2026-06-17)
Working on: the the external verifier fix-run handoff (.claude/plans/EXTVERIFIER-FIX-RUN-RUNBOOK.md). Branch
`fix/scoring-correctness-ext`. Fixing the 9 confirmed scoring-correctness bugs (F/I/D/E
mechanical + A/B/G/H/J decision-dependent) found via the the external-verifier method, each with a regression test.
Resolved the 4 owner-decisions in EXTVERIFIER-FIX-RUN-DECISIONS.md to the recommended answers (strict-caret,
...
**DONE + PUSHED** to branch `fix/scoring-correctness-ext` (NOT main; PR #119 open for operator
...
(the external verifier-gating) assessed + reported honestly in the PR as a separate cross-repo research follow-up,
...
NOT touching: keywords.rs/utils.rs/ace/{behavior/decay,context,db}.rs (PEER on fix/scoring-correctness-ext),
preemption.rs/blind_spots.rs (Phase 2, later).
Phase 0+1: DONE + PUSHED @ origin/main a43d8500 (clean FF). Migration phase 85.
Phase 2: DONE — committed @ 0fbdd1d7 on branch fix/scoring-correctness-ext (preemption.rs only,
...
the branch is @opus-scoring-external-verifier-fix's PR #119 — pushing my commit adds platform-deps into THEIR
...
FLAG: @opus-scoring-external-verifier-fix fixed my Phase-1 omission of target_cfg/platform_active from TEST-LOCAL
...
**DONE + PUSHED @ origin/fix/scoring-correctness-ext** (sync 0/0). Two commits: cf1375c8 (OSV
...
<!-- fable-public-surface (2026-06-12/13): DONE + PUSHED + LIVE-VERIFIED. Commit Lock RELEASED,
     ...
     CTAs -> module pages, sitemap 16 URLs. the external verifier task-a32de2f1 claimed but the board disconnected
...
<!-- fable-marketing-ledger (2026-06-12): DONE, entry cleared, terminal closed.
     ...
     + weekly digest scheduled tasks, the external verifier-contract Ed25519 attestation, CI verify on GitHub
...
<!-- opus-headless-engine (2026-06-10): DONE — committed LOCAL @ df62f45e (engine_runs.rs only, pathspec;
     push HELD for user). Ed25519-signed attribution receipts — the T2 forgery close (the external verifier M0.3 §F4 fix a).
     ...
     impact. @external-verifier-terminal: format CHOSEN + reference verifier + canonical contract handed off in
     .claude/plans/FOR-EXTVERIFIER-engine-attribution.md §2 + .claude/plans/verify-engine-receipt.mjs. Did NOT touch
...
<!-- opus-brief-quietfix (2026-06-10): DONE + PUSHED. Commit Lock RELEASED, claims cleared.
     ...
     + verifier freshness line ("Scanned N · X/Y sources · Zm ago" from latest engine_runs receipt, watermark
     fallback so it works with the external verifier/headless OFF) + abstention-contradiction fix (hide "no noteworthy" line
...
<!-- opus-headless-engine (2026-06-09) Phase 9: DONE + PUSHED @ e93e2f69 (origin, 0/0). Commit Lock RELEASED.
     ...
     column; scheduled/daemon/normal = NULL); the the external verifier-side fix for the replay free-ride gap (RESULTS-LOG §15).
     ...
     COORDINATION for the the external verifier terminal: .claude/plans/FOR-EXTVERIFIER-engine-attribution.md — (1) how to consume
     ...
     firing result. I did NOT touch their live forgery work or external-verifier repo. -->
...
<!-- opus-headless-engine (2026-06-09) LIVE TEST — scheduled-task FIRING verified (no commit, no files touched).
     @external-verifier-terminal: complements your Experiment A. You ran the engine MANUALLY via curl; I verified the
...
<!-- opus-headless-engine (2026-06-08) Phase 7: DONE + PUSHED @ 1cea4906 (origin, 0/0). Commit Lock RELEASED.
     ...
     getting large (413 lines), the external verifier live test (the external verifier terminal owns). dev `pnpm tauri dev` left running. -->
...
<!-- opus-headless-engine (2026-06-08) Phase 4: DONE + PUSHED. Commit Lock RELEASED, claims cleared.
     ...
     the external verifier live-test handoff written: .claude/plans/external-verifier-live-test-handoff.md (gitignored). REMAINING:
...
<!-- opus-headless-engine (2026-06-08): DONE. Phase 3 (bundling) committed LOCAL: 858f87e7 — `fourda
     ...
     Task Scheduler target = `fourda.exe --engine-once`; standalone fourda-engine.exe = dev/the external verifier console tool.
     ...
     wiring; the external verifier live test vs `fourda.exe --engine-once`. -->
     <!-- Commit Lock RELEASED (opus-headless-engine) -->
...
<!-- opus-headless-engine (2026-06-08): earlier phases. Both committed to main.
     ...
     entry; the external verifier live test against `fourda-engine --once`. Terminal idle. -->
     <!-- Commit Lock RELEASED (opus-headless-engine) -->
