...
function check8_privateAssetLeak(files, findings) {
  for (const f of files) {
    for (const hit of scanPrivateAssetLeakFile(REPO_ROOT, f)) {
      findings.push({ sev: hit.sev, rule: 'private-asset-leak', file: f, msg: hit.msg });
    }
  }
}
...
function main() {
  ...
  check8_privateAssetLeak(files, findings);
