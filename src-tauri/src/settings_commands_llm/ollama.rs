...
//! Ollama-specific logic: connection test, status check, model pulling.
...
pub(crate) async fn test_ollama_connection_impl(llm: &LLMProvider) -> Result<serde_json::Value> {
...
/// Implementation for cancel_ollama_pull command.
pub(super) fn cancel_ollama_pull_impl() -> Result<String> {
