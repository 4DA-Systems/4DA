...
pub struct DataFreshness {
    ...
    /// `engine_runs` receipt (the external verifier ground truth). Written by BOTH the in-app
    /// background scheduler AND the headless engine — so it is populated even when
    /// the external the external verifier verifier / headless task is disabled. None only when no
...
pub(crate) fn compute_data_freshness_from_conn(conn: &rusqlite::Connection) -> DataFreshness {
    ...
    // verifier freshness receipt — the most recent engine_runs row records what the last
...
pub(crate) fn build_enriched_briefing(
    ...
    // frontend pairs it with the verifier freshness line (data_freshness) so silence
