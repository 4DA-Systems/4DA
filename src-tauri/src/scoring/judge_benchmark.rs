...
//!
//! `benchmark_scenarios.json` already carries 87 human-labeled scenarios,
...
use super::benchmark_scenarios::{load_scenarios, profile_ctx, Scenario};
    ...
    for chunk in scenarios.chunks(BATCH_SIZE) {
        let items: Vec<ItemForJudgment> = chunk
            ...
            .map(|(idx, s)| ItemForJudgment {
                ...
                content: Some(s.item.content.clone()),
        ...
        for (idx, s) in chunk {
            ...
            outcomes.push(Outcome {
                ...
                title: s.item.title.chars().take(64).collect(),
...
async fn judge_accuracy_benchmark() {
    ...
    let mut by_profile: BTreeMap<String, Vec<(usize, &Scenario)>> = BTreeMap::new();
    ...
    // ── The gate ────────────────────────────────────────────────────────
    // Asserted on MCC, never on accuracy: 47 of 87 scenarios are labeled NOT
    // relevant, so a judge that rejects everything scores 0.54 accuracy while
    // carrying zero information. MCC 0.0 IS that judge.
    //
    // The floor is deliberately low. It is a REGRESSION tripwire ("the judge
    // still discriminates better than chance"), not a quality target — the
    // quality target is the operator's call once a baseline exists.
    assert!(
        m.total() > 0,
    ...
    assert!(
        m.mcc() > 0.0,
        "judge MCC {:.3} is at or below chance: it carries no usable information \
         about relevance, and every demotion it drives is unjustified",
        m.mcc()
    );
}
