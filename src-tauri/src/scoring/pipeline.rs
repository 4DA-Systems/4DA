...
use super::pipeline_signals::{self, SignalResult};
use super::*;
...
pub(crate) fn score_item(
    ...
    let signal = if options.apply_signals
        && relevant
        && combined_score >= 0.30
        ...
        && !show_and_tell_blocked
    {
        if let Some(clf) = classifier {
            pipeline_signals::classify_item_signal(
                input,
                ctx,
                db,
                clf,
    ...
    SourceRelevance {
        ...
        signal_type: signal.sig_type,
        signal_priority: signal.sig_priority,
        signal_action: signal.sig_action,
        signal_triggers: signal.sig_triggers,
        signal_horizon: signal.sig_horizon,
        similar_count: 0,
        similar_titles: vec![],
        serendipity: false,
        streets_engine,
