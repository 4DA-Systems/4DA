    ...
    fn setup_test_db() -> Connection {
        ...
        conn.execute_batch(
            "CREATE TABLE tech_stack (id INTEGER PRIMARY KEY, technology TEXT NOT NULL UNIQUE);
             ...
             CREATE TABLE project_dependencies (id INTEGER PRIMARY KEY, project_path TEXT, manifest_type TEXT, package_name TEXT, version TEXT, is_dev INTEGER DEFAULT 0, is_direct INTEGER DEFAULT 1, language TEXT, last_scanned TEXT DEFAULT (datetime('now')), detected_from TEXT, project_relevance REAL DEFAULT 1.0, UNIQUE(project_path, package_name));
             -- v29: build_domain_profile reads dependencies through the git-scoped
             -- temporal::get_all_dependencies, which joins user_dependencies and
             -- consults git_signals; the schema must carry both or the reader errors.
             CREATE TABLE user_dependencies (id INTEGER PRIMARY KEY, package_name TEXT, version TEXT, is_direct INTEGER DEFAULT 1, last_seen_at TEXT DEFAULT (datetime('now')));
             CREATE TABLE git_signals (id INTEGER PRIMARY KEY, repo_path TEXT, commit_hash TEXT, timestamp TEXT);
