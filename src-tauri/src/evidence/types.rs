...
pub struct UpgradePlanSnapshot {
    ...
    /// Oldest OSV ecosystem sync timestamp (`YYYY-MM-DD HH:MM:SS`, UTC) — the
    /// freshness FLOOR of the security data this plan reflects. `None` if never
    /// synced. A reader pairs this with `expires_at` to judge staleness.
    pub source_freshness: Option<String>,
    /// The engine run that produced this plan, when generated by the headless
    /// `fourda-engine` cycle. `None` when generated by the GUI's Signal-tier
    /// compute (no engine run). Correlates a plan to its receipt (`engine_runs`).
    pub engine_run_id: Option<i64>,
