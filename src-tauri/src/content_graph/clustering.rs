...
//! Community detection (deterministic Louvain-style local moving) and cluster
//! label extraction.
//!
...
pub(super) fn compute_clusters(items: &[RawItem], edges: &[GraphEdge]) -> Vec<GraphCluster> {
    ...
    // Louvain phase 1: local moves until stable.
    let mut community: Vec<usize> = (0..n).collect();
    let mut sigma_tot: Vec<f32> = degree.clone();
    ...
    for (i, &c) in community.iter().enumerate() {
        members.entry(c).or_default().push(order[i]);
    }
...
pub(super) fn assign_cluster_labels(items: &[RawItem], clusters: &mut [GraphCluster]) {
    ...
    for (cluster, tf) in clusters.iter_mut().zip(&tfs) {
        ...
        // Coverage: share of members whose title carries the top term.
        let coverage = scored
            .first()
            .map(|(top, _)| {
                let hits = cluster
                    ...
                    .count();
                hits as f32 / cluster.node_ids.len().max(1) as f32
            })
            .unwrap_or(0.0);

        cluster.label = if coverage >= LABEL_COVERAGE_MIN && !scored.is_empty() {
            scored
                .iter()
                .take(3)
                .map(|(w, _)| *w)
