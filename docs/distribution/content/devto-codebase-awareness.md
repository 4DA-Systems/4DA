...
The `@4da/mcp-server` package exposes 14 tools to any MCP-compatible AI assistant (Claude Code, Cursor, Windsurf, VS Code Copilot). These tools read from a local SQLite database populated by the 4DA desktop app, which runs the scanning and scoring in the background.
